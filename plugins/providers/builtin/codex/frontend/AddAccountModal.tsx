...
export function AddAccountModal({ open, api, onClose, onImported }: { open: boolean; api: AccountFeatureApi; onClose: () => void; onImported: (closeModal?: boolean) => Promise<void> }) {
  ...
  const run = async (action: (runId: number) => Promise<void>) => {
  ...
  const primaryLabel = busy
    ? (method === "oauth" ? "正在处理 OAuth 授权…" : "正在导入账号…")
    : method === "oauth"
      ? "在浏览器中继续"
      : "导入账号";
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="add-codex-account-title" aria-describedby="add-codex-account-description" aria-modal="true" className="codex-account-modal ow-native-dialog ow-modal-panel max-h-[90vh] overflow-hidden rounded-[28px] p-0" onCancel={(event) => { event.preventDefault(); close(); }}>
      <div className="codex-account-modal-header flex items-start justify-between gap-4 px-5 pb-4 pt-5 sm:px-7 sm:pt-6">
        <div className="min-w-0">
          <h2 id="add-codex-account-title" className="text-xl font-extrabold tracking-[-0.03em] text-[var(--ow-text)]">添加账号</h2>
          <p id="add-codex-account-description" className="mt-1.5 text-sm leading-6 text-[var(--ow-muted)]">选择一种方式，将凭据安全地加入本地账号库。</p>
        </div>
        <button type="button" aria-label="关闭添加账号弹窗" className="codex-account-modal-close ow-btn shrink-0 rounded-xl px-3 py-2 text-xs font-semibold" onClick={close}>关闭</button>
      </div>

      <div className="codex-account-modal-tabs ow-segment mx-5 grid grid-cols-2 rounded-2xl p-1 sm:mx-7" role="tablist" aria-label="账号导入方式">
      ...
      <div className="codex-account-modal-body overflow-y-auto px-5 py-5 sm:px-7 sm:py-6">
        ...
        {method === "oauth" && (
          <section id="add-account-oauth-panel" role="tabpanel" aria-labelledby="add-account-oauth-tab" className="codex-account-modal-mode ow-page-frame-soft rounded-[22px] p-5 shadow-none">
            <span className="codex-account-method-badge">推荐方式</span>
            <h3 className="mt-3 text-lg font-extrabold tracking-[-0.02em] text-[var(--ow-text)]">使用浏览器授权</h3>
            <p className="mt-2 text-sm leading-6 text-[var(--ow-muted)]">我们会打开系统浏览器完成 OpenAI 授权，并自动接收本地回调。账号导入后不会自动切换。</p>
            <div className="codex-account-oauth-facts mt-5 grid gap-2 sm:grid-cols-3">
              <div><span>授权窗口</span><strong>系统浏览器</strong></div>
              <div><span>回调方式</span><strong>本地自动接收</strong></div>
              <div><span>导入之后</span><strong>手动应用</strong></div>
            </div>
            <details className="codex-account-modal-fallback mt-5 rounded-2xl" open={showFallback} onToggle={(event) => setShowFallback(event.currentTarget.open)}>
              <summary className="cursor-pointer px-4 py-3.5 text-sm font-semibold text-[var(--ow-text)]">没有收到回调？</summary>
              <div className="border-t border-[var(--ow-line-soft)] px-4 pb-4 pt-3">
                ...
                <label className="mt-3 block text-xs font-semibold text-[var(--ow-text)]">回调 URL
                  <input type="url" value={callbackUrl} onChange={(event) => setCallbackUrl(event.target.value)} className="codex-account-field mt-2 w-full rounded-xl px-3 py-2.5 text-sm" placeholder="http://localhost:1455/auth/callback?..." autoComplete="off" spellCheck={false} />
                </label>
                <div className="mt-3 flex justify-end">
                  <button type="button" disabled={!callbackUrl.trim() || (busy && !loopbackHandle)} className="ow-btn rounded-xl px-3 py-2 text-xs font-bold" onClick={submitCallback}>使用回调 URL</button>
                </div>
              </div>
            </details>
          </section>
        ...
        {method === "token" && (
          <section id="add-account-token-panel" role="tabpanel" aria-labelledby="add-account-token-tab" className="codex-account-modal-mode ow-page-frame-soft rounded-[22px] p-5 shadow-none">
            <span className="codex-account-method-badge">本地导入</span>
            <h3 className="mt-3 text-lg font-extrabold tracking-[-0.02em] text-[var(--ow-text)]">粘贴 Token 或账号 JSON</h3>
            ...
            <label className="mt-4 block text-xs font-semibold text-[var(--ow-text)]">Token / 账号 JSON
              <textarea value={content} onChange={(event) => setContent(event.target.value)} className="codex-account-field mt-2 min-h-40 w-full resize-y rounded-xl p-3.5 font-mono text-xs leading-5" placeholder="粘贴账号对象或数组" spellCheck={false} />
            </label>
          </section>
        )}
      ...
      <div className="codex-account-modal-footer flex flex-col gap-3 border-t border-[var(--ow-line-soft)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
        <p role="status" className="min-h-5 text-xs text-[var(--ow-muted)]">{busy ? primaryLabel : ""}</p>
        <div className="flex flex-col-reverse gap-2 sm:flex-row">
          <button type="button" className="ow-btn rounded-xl px-4 py-2.5 text-sm font-semibold" onClick={close}>取消</button>
