...
推送版本 tag（例如 `1.2.0`）也会通过 `.github/workflows/release-dmg.yml` 自动构建同一条 Apple Silicon DMG 链路。workflow 会先上传一份 Actions artifact；如果对应 GitHub Release 不存在，会先自动创建，再把 DMG 追加到该 Release 的资产列表。
