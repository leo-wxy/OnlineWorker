...
**Areas discussed:** 主题切换规则与入口、暗色视觉基调、覆盖与系统联动、后续 UI 复用与开发文档
...
|-------------|----------|
| 设计一套可复用主题结构，使后续新增 UI 直接消费统一 token | ✓ |
| 将主题规则写入正式开发文档，作为后续 UI 开发参考 | ✓ |

**User's choice:** 两项均为明确新增要求。
**Notes:** 主题结构保持轻量，不引入主题库或完整组件库；正式规范使用单一稳定文档并从现有文档索引进入，避免在 README、CONTRIBUTING 和 Phase 资料中重复维护同一套规则。
