...
- [x] 17-01: Isolate provider session logic from core
  - [x] Inventory provider-specific Tauri/core session logic.
  - [x] Define provider-owned session/facts boundary for App, TaskBoard/Dashboard, and Telegram.
  - [x] Migrate Claude session/workspace reads behind provider/plugin-owned capabilities.
  - [x] Remove duplicated Claude filtering from core.
  - [x] Verify the shared source contract and installed provider facts; record the user-approved live Telegram waiver without claiming a TG pass.

Latest verification:

- Phase 17 closed on 2026-07-11 with canonical `17-VERIFICATION.md` status `passed` and `17-UAT.md` complete. Full gates passed (`996` Python, `206` Rust, `159` frontend Node, production frontend build), version `1.7.4` built/installed/relaunched, IPC and binary identity checks passed, and the installed generic owner bridge returned real Claude/Codex provider facts. Real Telegram `/workspace` visual comparison was explicitly waived by the user and is not represented as passed.
...
- Phase 18 closed on 2026-07-11 with canonical `18-VERIFICATION.md` status `passed` and `18-UAT.md` complete. The packaged App owner-bridge path returned pending and then materialized real Codex and Claude sessions with their first messages. Real Telegram `/new` UAT was explicitly waived by the user and is not represented as passed. The observed failed/stalled provider turns are assigned to Phase 19 interruption/recovery work.

### Phase 19: Attention Center And Session Interrupt/Resume

**Goal:** Give users one focused place to handle actionable pending states and let them interrupt, resume, or recover real provider Sessions without creating local-only state.
**Requirements**: TBD
**Depends on:** Phase 18
**Scope Fence:** This phase covers the pending-action center plus Session interrupt/resume/recovery. It does not add global search. Approval actions must preserve the existing app-server request/reply authority and mirrored-only items must remain observational.
**Plans:** 2 plans
...
- [x] 24-01-PLAN.md — 限定 MessageEventBus 去重保留窗口（source verified）
- [x] 24-02-PLAN.md — 保留 last-known-good 并区分空结果与读取失败（source verified）
- [x] 24-03-PLAN.md — 收敛 canonical publish、turn 顺序和前端 snapshot/delta 仲裁（source verified）
...
3. 旧 turn、旧 generation 和迟到请求不能回退当前 session、stream、service 或 Menubar 状态。
