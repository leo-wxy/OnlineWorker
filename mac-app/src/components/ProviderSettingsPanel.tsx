  ...
  if (iconUrl) {
    return (
      <img
        src={iconUrl}
        alt=""
        className="h-11 w-11 shrink-0 rounded-2xl border border-[var(--ow-line)] bg-[var(--ow-panel)] object-contain p-1.5 shadow-sm"
      />
    );
  }
  const label = provider?.label?.trim() || provider?.id || "?";
  return (
    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-[var(--ow-line)] bg-[var(--ow-panel)] text-sm font-bold text-[var(--ow-muted)] shadow-sm">
...
function providerSettingClass(enabled: boolean) {
  return enabled
    ? "border-[var(--ow-blue)] bg-[var(--ow-blue-soft)]"
    : "border-[var(--ow-line)] bg-[var(--ow-panel-soft)] opacity-75";
}

function validationToneClass(report: ProviderValidationReport) {
  if (!report.ok) {
    return "border-[var(--ow-red)] bg-[var(--ow-red-soft)] text-[var(--ow-error-text)]";
  }
  if (report.checks.some((check) => check.severity === "warning" && !check.ok)) {
    return "border-[var(--ow-amber)] bg-[var(--ow-amber-soft)] text-[var(--ow-warning-text)]";
  }
  return "border-[var(--ow-green)] bg-[var(--ow-green-soft)] text-[var(--ow-green)]";
}
...
function validationCheckDotClass(severity: string, ok: boolean) {
  ...
  if (severity === "warning") {
    return "bg-[var(--ow-amber)]";
  }
  return "bg-[var(--ow-green)]";
}
...
export function ProviderSettingsPanel({ mode }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-extrabold tracking-[-0.02em] text-[var(--ow-text)]">{title}</h2>
        <p className="mt-1 text-sm text-[var(--ow-muted)]">{description}</p>
      </div>

      {loading && (
        <div className="rounded-2xl border border-[var(--ow-line)] bg-[var(--ow-panel)] p-5 text-sm text-[var(--ow-muted)]">
          {texts.loading}
        </div>
      )}

      {error && (
        <div className="rounded-2xl border border-[var(--ow-red)] bg-[var(--ow-red-soft)] p-4 text-sm text-[var(--ow-error-text)]">
          {error}
        </div>
      )}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-base font-bold text-[var(--ow-text)]">{setting.label}</h3>
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${
                        !cliAvailable
                          ...
                          : "bg-[var(--ow-disabled-surface)] text-[var(--ow-muted)]"
                      }`}>
                        {!cliAvailable ? texts.cliMissing : setting.enabled ? texts.enabled : texts.disabled}
                      </span>
                      {hiddenByDefault && (
                        <span className="rounded-full bg-[var(--ow-purple-soft)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--ow-purple)]">
                    ...
                    <p className="mt-2 truncate text-xs font-mono text-[var(--ow-muted)]" title={provider?.bin ?? undefined}>
                      {provider?.bin ?? provider?.install?.cliNames?.[0] ?? setting.id}
                    </p>
                    {hiddenByDefault && (
                      <p className="mt-2 text-xs font-medium text-[var(--ow-purple)]">
                        {texts.hiddenByDefaultHint}
                      </p>
                    )}
                    {!cliAvailable && (
                      <p className="mt-2 text-xs font-semibold text-[var(--ow-warning-text)]">
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                  {busy && <span className="text-xs font-semibold text-[var(--ow-blue)]">{texts.saving}</span>}
                  <button
                    type="button"
              ...
              <div className="mt-4 flex flex-wrap items-center gap-6">
                <label className={`flex items-center gap-3 ${busy || !canEnable ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
                  ...
                  />
                  <span className="text-sm font-semibold text-[var(--ow-text)]">{texts.enable}</span>
                </label>
              ...
              {report && (
                <div className={`mt-5 rounded-xl border p-4 ${validationToneClass(report)}`}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className="rounded-full bg-[var(--ow-panel)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] opacity-80">
                      {report.status}
                    </span>
                  </div>
                  <div className="mt-3 grid gap-2">
                    {report.checks.map((check) => (
                      <div key={check.id} className="flex gap-2 rounded-lg bg-[var(--ow-panel)] px-3 py-2 text-xs">
              ...
              {provider && (supportsExternalCliRewrite || showManagedRemoteProxyAlias || canEditLaunchMethods) && (
                <div className="mt-5 grid gap-3 rounded-xl border border-[var(--ow-line)] bg-[var(--ow-panel)] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <h4 className="text-sm font-bold text-[var(--ow-text)]">{texts.cliConfigTitle}</h4>
                    {cliBusy && <span className="text-xs font-semibold text-[var(--ow-blue)]">{texts.saving}</span>}
                  </div>
                  {showManagedRemoteProxyAlias && (
                    <div className="grid gap-2 border-l-2 border-[var(--ow-blue)] pl-3">
                      <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                          <div className="text-xs font-bold text-[var(--ow-blue)]">{texts.externalCliProxyAliasTitle}</div>
                          <div className="mt-0.5 text-xs font-medium text-[var(--ow-blue)]">{texts.externalCliProxyAliasDescription}</div>
                        </div>
                        <CopyCommandButton text={remoteProxyAlias} />
                      </div>
                      <code className="block break-all rounded-md bg-[var(--ow-code)] px-2.5 py-2 text-xs font-semibold text-[var(--ow-text)]">
                        {remoteProxyAlias}
                      </code>
                    </div>
                  )}
                  {(supportsExternalCliRewrite || canEditLaunchMethods) && (
                    <>
                      <div className="grid gap-3">
                        <label className="grid gap-1.5 text-xs font-bold text-[var(--ow-muted)]">
                        ...
                        {canEditLaunchMethods && (
                          <p className="text-xs font-medium leading-5 text-[var(--ow-muted)]">
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-3">
                        {supportsExternalCliChildLauncher && (
                          <label className={`mr-auto flex items-center gap-3 ${cliBusy ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
                            ...
                            />
                            <span className="text-sm font-semibold text-[var(--ow-text)]">{texts.externalCliLaunchesManagedChildCli}</span>
                          </label>
                        )}
                        <button
                          type="button"
