...
test("generic provider chat keeps header state aligned without remounting on live list updates", () => {
  ...
  assert.match(genericChat, /useEffect\(\(\) => \{\s*liveStreamReadyRef\.current = false;\s*(?:localUserTurnsRef\.current = \[\];\s*)?setActiveSession\(session\);\s*\}, \[session\.id, session\.type, session\.workspace\]\);/s);
