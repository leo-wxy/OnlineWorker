...
test("loadSnapshotIfCurrent drops a snapshot superseded by a live update", async () => {
  let generation = 1;
  let resolveSnapshot;
  const pending = loadSnapshotIfCurrent(
    () => new Promise((resolve) => {
  ...
  generation += 1;
  resolveSnapshot([{ role: "assistant", content: "stale" }]);
